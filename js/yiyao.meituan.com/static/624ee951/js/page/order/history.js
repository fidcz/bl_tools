...
define('components/cascader_select', [], function() {
    return {
        props: {
            value: {
                "default": -1
        ...
        watch: {
            ...
            city: function(i) {
                var t = this.poiMap[this.city] || [];
                this.poi = t.length && t[0].wm_poi_id,
                this.width = 14 * this.city.length + 50 + "px"
            },
            poi: function(i) {
                this.poiChange(i)
            }
        ...
        methods: {
            poiChange: function(i) {
                this.query = "",
                this.$emit("input", i),
                this.$emit("change", i)
            },
            getPoiName: function(i) {
                return i.businessDesc ? i.name + " [" + i.businessDesc + "]" : i.name
            }
        },
        mounted: function() {
            var i = this;
            i.poi = this.value,
            i.$nextTick(function() {
                var t = i.$refs.poiSelect
                  , e = t.$refs.kuiSelectInput.$el.querySelector("input");
                e.addEventListener("focus", function(t) {
                    i.$refs.poiSelect.filterText = null,
                    i.isFocus = !0
                }),
        ...
        template: '<div class="cascader-search inline-block"><kui-select class="city-select" :options="cityList" v-model="city" v-if="cityList.length > 2" :style="{ \'width\': width }"></kui-select><kui-select  class="poi-select" :options="poiMap[city]" :props="poiProps" ref="poiSelect"v-model="poi" @change="poiChange" placeholder="输入门店名称即可快速查询" filterable><kui-option v-for="option in poiMap[city]":key="option.wm_poi_id":label="getPoiName(option)":value="option.wm_poi_id":class="{ gray: option.businessDesc !== \'营业中\'}"></kui-option></kui-select><kui-tooltip manual :visible="isFocus">输入门店名称即可快速查询</kui-tooltip></div>'
    }
});
require.config({
    baseUrl: VM.STATIC_URL,
...
require(["module/general", "util/utils", "module/order/order_list", "module/order/order_slidebar", "module/interface", "components/cascader_select", "module/cookie", "module/bridge/bridge_app"], function(e, t, a, o, n, r, i, s) {
    function c() {
        var e = t.getOffsetDays(v.val(), f.val())
          , a = "food" == CommConstants.shopCate
          , o = a ? 5 : 7
          , n = a ? "只显示近" + o + "天订单信息，请重新选择日期" : "查询时间段限制在" + o + "天以内，请重新选择日期";
        return e > o ? (t.showTip(n, "warn"),
        !1) : !0
    }
    function l() {
        a.init({
            page: "history",
            queryUrl: n.order.queryHisOrderList,
            getQueryFields: function() {
                var e = {};
                return e.wmPoiId = u,
                e.acctId = d,
                e.brandId = parseInt(i.getCookie("brandId") || 0),
                e.isChain = localStorage && localStorage.getItem("isChain"),
                e.requestWmPoiId = m.poi,
                e.wmOrderPayType = $('input[name="pay-type"]:checked').val(),
                e.wmOrderStatus = $('input[name="order-status"]:checked').val(),
                e.sortField = $('input[name="rank-item"]:checked').val(),
                e.size = "",
                e.warehouseId = h.warehouseId,
                e.lastLabel = w,
                e.nextLabel = y,
                e.startDate = v.val() ? v.val() : "",
                e.endDate = f.val() ? f.val() : "",
                e.getNewVo = 1,
                e
            },
            bindQueryEvts: function() {
                var e = $('input[name="pay-type"], input[name="order-status"], input[name="rank-item"]');
                e.on("click", function(e) {
                    w = "",
                    y = "",
                    c() && l();
                    var a = e.currentTarget.name
                      , o = e.currentTarget.value;
                    "order-status" == a ? t.reportMC("b_shangou_online_e_oppkvbta_mc", "c_nehoktcu", null, null, {
                        button_nm: o
                    }) : "pay-type" == a ? t.reportMC("b_shangou_online_e_opahodve_mc", "c_nehoktcu", null, null, {
                        button_nm: o
                    }) : "rank-item" == a && t.reportMC("b_shangou_online_e_o7fo1h0g_mc", "c_nehoktcu", null, null, {
                        button_nm: o
                    })
                }),
                I.on("dp.change", function() {
                    b = t.changeDate(1),
                    k.data("DateTimePicker").maxDate(b),
                    k.data("DateTimePicker").minDate(v.val()),
                    _.data("DateTimePicker").maxDate(f.val()),
                    w = "",
                    y = "",
                    c() && (t.reportMC("b_shangou_online_e_ndm1fp5b_mc", "c_nehoktcu", null, null, {
                        select_date: v.val() + "至" + f.val()
                    }),
                    l())
                })
            },
            afterLoaded: function() {
                ...
                "" === localStorage.getItem("lastLabel") && $(".J-prev-page").attr("disabled", "disabled"),
                "" === localStorage.getItem("nextLabel") && $(".J-next-page").attr("disabled", "disabled")
            }
        }, !0)
    }
    var u = Number(i.getCookie("wmPoiId")) || -1
      , d = i.getCookie("acctId")
      , p = i.getCookie("token")
      , m = new Vue({
        el: "#poi-wrap",
        components: {
        ...
        computed: {
            poiList: function() {
                var e = [];
                for (var t in this.options)
                    e = e.concat(this.options[t]);
                return e
            },
            showPoiWrap: function() {
                return this.poiList.length > 1
            },
            poiName: function() {
                var e = ""
                  , t = this;
                return -1 == t.poi ? "全部门店" : (t.poiList.forEach(function(a) {
                    return t.poi == a.wm_poi_id ? e = a.name : void 0
                }),
                e)
            }
        },
        ...
        methods: {
            loadData: function() {
                var e = this;
                this.$http.get("/v2/order/history/r/queryCityAndWmPoi").then(function(t) {
                    var a = t.body;
                    a.code ? e.$toast.error(a.msg) : e.options = a.data
                })
            }
        },
        watch: {
            poi: function(e) {
                console.log("历史订单选择门店id:", e),
                w = y = "";
                try {
                    h.loadData(e)
                } catch (a) {
                ...
                t.reportMC("b_shangou_online_e_jra6cdyz_mc", "c_nehoktcu")
            }
        }
    })
      , h = new Vue({
        ...
        computed: {
            showWarehouseWrap: function() {
                return this.options.length > 1 && m.poi > -1 ? !0 : !1
            }
        },
        mounted: function() {
            this.loadData()
            ...
            loadData: function(e) {
                ...
                this.$http.get("/sc/retail/express/get/warehouseInfos", {
                    ...
                    if (a.code)
                        ...
                        console.log(a.data);
                        var o = a.data || [];
                        t.options = o.map(function(e, t) {
                            return {
                                value: e.id,
                                label: e.name
                            }
                        }),
                        t.options.splice(0, 0, {
                            value: -1,
                            label: "全部"
                        })
                    }
    ...
    new Vue({
        ...
        mounted: function() {
            this.checkExpressAbility()
        },
        methods: {
            checkExpressAbility: function() {
                var e = this;
                $.ajax({
                    type: "GET",
                    url: n.order.sc.batchDeliveryCapbility,
                    data: {
                        acctId: d,
                        wmPoiId: u,
                        token: p
                    ...
                    success: function(t) {
                        0 == t.code ? e.isSupportExpress = t.data : e.$toast.error(t.msg)
                    }
                })
            },
            jumpToExpress: function() {
                s.jumpTo({
                    href: "/sc/retail/express/uploadinfo"
                })
            }
        }
    }),
    "flag" === location.search.substring(1) && $(".invalid").prop("checked", !0);
    var g = localStorage.getItem("orderHisRank");
    g && $('input[name="rank-item"]').eq(g - 1).prop("checked", !0),
    $('input[name="rank-item"]').change(function() {
        localStorage.setItem("orderHisRank", $('input[name="rank-item"]:checked').val())
    });
    var v = $("#J-start")
      , f = $("#J-end")
      , _ = $(".J-start-date")
    ...
    $(".J-next-page").click(function() {
        w = "",
        y = localStorage.getItem("nextLabel"),
        c() && l()
    }),
    $(".J-prev-page").click(function() {
        w = localStorage.getItem("lastLabel"),
        y = "",
        c() && l()
    }),
    t.reportPV("c_nehoktcu", {
        des: "PC历史订单"
    })
});
define("page/order/history", function() {});
